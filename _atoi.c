#include "shell.h"

/**
 * interactive – I am returning functions from the shell in interactive
 * @info: struct address
 * Authors: Moshe Matabologa and Onkabetse02
 * Return: 1 if interactive mode, 0 otherwise for flagging
 */
int interactive(info_t *info)
{
	return (isatty(STDIN_FILENO) && info->readfd <= 2);
}

/**
 * is_delim – is going to check if character is delimiter
 * @c: is going to check the char
 * @delim: will reference the delimeter string
 * Return: 1 if true, 0 if false
 */
int is_delim(char c, char *delim)
{
	while (*delim)
		if (*delim++ == c)
			return (1);
	return (0);
}

/**
 *_isalpha - checks for alphabetic character
 *@c: The character to input
 *Return: 1 if c is alphabetic, 0 otherwise
 */

int _isalpha(int c)
{
	if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z'))
		return (1);
	else
		return (0);
}

/**
 *_atoi – will convert strings to intergers
 *@s: strings will be converted
 *Return: 0 if no numbers in string, converted number otherwise
 */

int _atoi(char *s)
{
	int i, sign = 1, flagging = 0, output;
	unsigned int result = 0;

	for (i = 0;  s[i] != '\0' && flagging != 2; i++)
	{
		if (s[i] == '-')
			sign *= -1;

		if (s[i] >= '0' && s[i] <= '9')
		{
			flagging = 1;
			result *= 10;
			result += (s[i] - '0');
		}
		else if (flagging == 1)
			flagging = 2;
	}

	if (sign == -1)
		output = -result;
	else
		output = result;

	return (output);
}
